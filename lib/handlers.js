...
const handlers = {
  ...
  user: ({method, trimmedPath, ...dataToHandle}, callback) => {
    ...
    if (!supportedMethods.includes(method)) callback({ error: `HTTP method ${method} is not supported for /${trimmedPath}!` }, 405);
  ...
  userHandlersByMethod: {
    ...
    POST: ({payload}, callback) => {
      ...
      if (validators.some(validator => !validator)) return callback({ error: 'Missing or invalid required fields!' }, 400);
